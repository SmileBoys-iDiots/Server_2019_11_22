	...
	public int calcPcPcDamage(int adddmg) {
		...
		if (_target != null) {
			...
			for (L1ItemInstance item : _targetPc.getInventory().getItems()) {
				...
				//할파스 의갑옷 (완력,예지력,마력)
				else if (item.getItemId() >= 5000000
						&& item.getItemId() <= 5000002) {
					int addchan = item.getEnchantLevel() - 0;
					if (addchan < 0)
						addchan = 0;
					if (chance1 < 6 + addchan) {
						if (_targetPc.getSkillEffectTimerSet()
								.hasSkillEffect(L1SkillId.할파스가호lv1)) {
							break;
						}
						
						_targetPc.getSkillEffectTimerSet().setSkillEffect(
								L1SkillId.할파스가호lv1,  4000 );  
						
						if (item.getItemId() == 5000000)// 완력
							_targetPc.setCurrentMp(_targetPc.getCurrentMp()
									+ 15 + _random.nextInt(7));
						else if (item.getItemId() == 5000001)// 예지
							_targetPc.setCurrentMp(_targetPc.getCurrentMp()
									+ 20 + _random.nextInt(7));
						else if (item.getItemId() == 5000002)// 인내
							_targetPc.setCurrentMp(_targetPc.getCurrentMp()
									+ 25 + _random.nextInt(9));
						if (item.getItemId() == 5000000)// 완력
							_targetPc.setCurrentHp(_targetPc.getCurrentHp()
									+ 100 + _random.nextInt(7));
						else if (item.getItemId() == 5000001)// 예지
							_targetPc.setCurrentHp(_targetPc.getCurrentHp()
									+ 100 + _random.nextInt(7));
						else if (item.getItemId() == 5000002)// 인내
							_targetPc.setCurrentHp(_targetPc.getCurrentHp()
									+ 100 + _random.nextInt(9));

						_targetPc.sendPackets(
								new S_SkillSound(_targetPc.getId(), 19068),
								true);
						Broadcaster.broadcastPacket(_targetPc,
								new S_SkillSound(_targetPc.getId(),19068),
								true);
